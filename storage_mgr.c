    ...
    if( newFile != 0){
        ...
        // seek set right or not
            ...
            if( pageSize != 0){
                ...
                size_t modPageNum = pageSize % BLOCK_SIZE; 
                ...
                */
                ...
                if (modPageNum == 0){
                    fHandle->totalNumPages =(int) pageSize / BLOCK_SIZE;    // test 3: size_t mod num 
                }
                else{
                    fHandle->totalNumPages = (int) (pageSize / BLOCK_SIZE + 1);
                }
    ...
    size_t readStatement;
		...
		if (memPage = NULL){
			return RC_mempage_null;
                ...
                memPage = (char *)malloc(sizeof(BLOCK_SIZE));
		...
		if (fseek(fp, (BLOCK_SIZE*(pageNum)) % PAGE_SIZE, SEEK_SET) == 0){

		    fread(memPage,sizeof(char),BLOCK_SIZE,fp);
		 }
                rewind(fp);
                fclose(fp);
                return RC_OK;
...
RC writeBlock (int pageNum, SM_FileHandle *fHandle, SM_PageHandle memPage) {
 ...
 //    }
    ...
    setSuccess = fseek(fp, sizeof(char) * BLOCK_SIZE * (pageNum), SEEK_SET);
    if (setSuccess != 0) {
        return RC_SET_POINTER_FAILED;
    }

 //    // fix fHandle-> mgmInfo to fp; Pingyu Xue  0926
    writeState = fwrite(memPage, sizeof(char), BLOCK_SIZE, fp);
